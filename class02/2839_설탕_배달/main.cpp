...
정확하게 N킬로그램을 배달해야 한다. 설탕공장에서 만드는 설탕은 봉지에 담겨져
있다. 봉지는 3킬로그램 봉지와 5킬로그램 봉지가 있다.
...
int main() {
	...
	while (fiveKiloEnv >= 0) {
		threeKiloEnv = 0;
		while (fiveKiloEnv * 5 + threeKiloEnv * 3 < n)
			threeKiloEnv++;
		if (fiveKiloEnv * 5 + threeKiloEnv * 3 == n)
			break;
		fiveKiloEnv--;
	}
	if (fiveKiloEnv == -1)
		...
		std::cout << fiveKiloEnv + threeKiloEnv << '\n';
