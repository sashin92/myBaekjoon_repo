...
int main() {
    ...
    int oneToN = 1;
    stack<int> stk;
    vector<bool> checkPushPop;
    for (auto node: vec) {
        while (oneToN <= node) {
            stk.push(oneToN++);
            checkPushPop.push_back(true);
        }
        if (stk.top() == node) {
            stk.pop();
            checkPushPop.push_back(false);
        }
        else {
            cout << "NO\n";
            return 0;
...
/*
내 수는 1부터 n까지.
target까지 push 후 pop, 다음 타겟까지 push 후 pop
끝을 찍기 전까지 내림차순은 거리가 1이 아니면 안됨
끝을 찍고나면 pop만 할 수 있다
